...
import { saveInferMetadata, saveProviderConfig, setActiveProvider } from '@/lib/db/mutations'
...
export default function SettingsPage() {
  ...
  async function handleInferMetadataToggle(value: boolean) {
    await saveInferMetadata(value)
  }
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          >
            {isDark ? (
              ...
              <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            )}
            {isDark ? 'Light' : 'Dark'}
          </button>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {(() => {
            ...
            return (
              <div className="card p-5 md:p-6 border-primary-500 dark:border-primary-500 relative overflow-hidden">
                ...
                <div className="mb-5">
                  <label htmlFor={`${p}-api-key`} className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1.5">
                    API Key
                  </label>
                ...
                <div className="mb-6">
                  <label htmlFor={`${p}-model`} className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1.5">
                    Model
                  </label>
                  <select
                    id={`${p}-model`}
          ...
          <div className="flex flex-col items-center gap-4 mt-2">
            ...
            {status[activeProvider] && (
              ...
              >
                {status[activeProvider] === 'Connected!' ? (
                  <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="animate-check-bounce">
          ...
          <div className="card p-5 md:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold mb-2">AI Behavior</h2>
                <p className="text-sm text-surface-500 dark:text-surface-400">
                  Control how proactively the assistant fills in metadata for your todo items.
                </p>
              </div>
              ...
              <label className="relative inline-flex cursor-pointer items-center">
                <input
                  type="checkbox"
                  checked={settings?.inferMetadata ?? false}
                  onChange={(e) => handleInferMetadataToggle(e.target.checked)}
                  className="peer sr-only"
                />
                <div className="h-6 w-11 rounded-full bg-surface-300 transition-colors peer-checked:bg-primary-600 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary-500 dark:bg-surface-700" />
            ...
            <div className="mt-5 rounded-xl border border-surface-200 bg-surface-50 p-4 dark:border-surface-800 dark:bg-surface-900/60">
              <div className="text-sm font-medium text-surface-800 dark:text-surface-100">
                Auto-suggest priority & effort
              </div>
              <p className="mt-1 text-sm text-surface-500 dark:text-surface-400">
                When enabled, the AI will automatically infer and suggest priority and effort levels for new items.
              </p>
            </div>
          </div>
        </div>
