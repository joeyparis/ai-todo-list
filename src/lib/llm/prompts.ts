...
export function buildSystemPrompt(list: ListContext, items: ItemContext[]): string {
  ...
  return [
    ...
    '',
    '3) Completion command -> completeItems',
    'User: "Mark groceries as done."',
    'Assistant reasoning: imperative completion command against an existing item.',
    'Assistant tool call: completeItems',
    '{"itemIds":["a1b2c3"]}',
    'Assistant (after tool): "Done - checked that off."',
    '',
    '4) Already done but not on list -> addAndCompleteItems',
    ...
    '',
    '5) Planning question -> smart response, no tool',
    ...
    '',
    '6) Pronoun completion -> ask clarifying question',
    'User: "That\'s done."',
    'Assistant: "Which task did you finish? I want to make sure I mark the right one."',
    '',
    '7) Completion with "also" modifier -> completeItems (not confused by "also")',
    'User: "I also finished the groceries."',
    'Assistant reasoning: verb is "finished" (completion), "also" is a modifier, not an addition signal.',
    'Assistant tool call: completeItems',
    '{"itemIds":["a1b2c3"]}',
    'Assistant (after tool): "Great - groceries are checked off."',
    '',
    '8) Ambiguous match clarification -> ask before tool call',
    'User: "I did the laundry."',
    'Assistant reasoning: active items include both "Do laundry" and "Pick up laundry from cleaners" - ambiguous.',
    'Assistant: "Did you do the laundry at home or pick up from the cleaners? I want to mark the right one."',
    '',
    '9) Already-completed item -> acknowledge, no tool call',
    'User: "Mark groceries as done."',
    'Assistant reasoning: "Buy milk and eggs" is already [done] in current list context.',
    'Assistant: "That\'s already checked off - you got it done earlier."',
    '',
