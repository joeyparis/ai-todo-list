...
function buildItemInputSchema(inferMetadata: boolean) {
  return z.array(z.object({
    ...
    metadata: coreMetadataSchema.optional().describe(
      inferMetadata
        ? `Optional inferred metadata using only these keys and values: ${CORE_METADATA_RULES}`
        : `Metadata for the item - only set when the user explicitly requests it. Allowed values: ${CORE_METADATA_RULES}`
    ),
  }))
}
...
export function buildTodoTools(inferMetadata = false) {
  const itemInputSchema = buildItemInputSchema(inferMetadata)
  return {
    addItems: tool({
      description: inferMetadata
        ? `Add one or more new todo items to the current list. Use this when the user describes tasks, activities, or things they need to do. Infer metadata from context when possible using only this core schema: ${CORE_METADATA_RULES}.`
        : `Add one or more new todo items to the current list. Use this when the user describes tasks, activities, or things they need to do. Only set metadata when the user explicitly requests it.`,
...
export const todoTools = buildTodoTools(false)

export type TodoTools = ReturnType<typeof buildTodoTools>
